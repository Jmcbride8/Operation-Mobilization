...
export default function TurnaroundGate({ onUnlock }) {
  ...
  const handleSubmit = (e) => {
    ...
    if (password === "resurgent") {
      sessionStorage.setItem("turnaround_access", "granted");
      onUnlock();
    } else {
      setError(true);
    }
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="flex items-center gap-2 text-titanium">
            <Lock size={16} />
            <span className="text-[10px] font-mono tracking-[0.2em]">CLASSIFIED — RESTRICTED ACCESS</span>
          </div>
          <input
            type="password"
            value={password}
